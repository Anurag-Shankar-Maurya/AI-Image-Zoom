...
import type { Rect, HistoryStep, ImageDescription, ImageDisplayHandle } from './types';
...
const App: React.FC = () => {
  ...
  const resetState = useCallback(() => {
    ...
    setDisplaySelection(null);
    imageDisplayRef.current?.resetZoom();
    loadInitialImage();
  }, [loadInitialImage]);
  ...
  const runEnhancementJob = useCallback(async () => {
    ...
    try {
        ...
        const enhancedPaddedSrc = await serviceEnhance(croppedForEnhancement, [...descriptionHistory.map(d=>d.prompt), description.prompt]);
  ...
  return (
    ...
    >
      ...
      {appState === AppState.LOADED && finalImageSrc && (
        <div className="absolute top-4 right-4 z-20 flex flex-col items-center gap-2 bg-black/50 p-2 rounded-md border border-green-500/60">
          <button onClick={() => imageDisplayRef.current?.zoomIn()} className="w-8 h-8 text-green-400 hover:bg-green-500/20 rounded transition-colors text-lg" aria-label="Zoom In">+</button>
          <button onClick={() => imageDisplayRef.current?.zoomOut()} className="w-8 h-8 text-green-400 hover:bg-green-500/20 rounded transition-colors text-lg" aria-label="Zoom Out">-</button>
          <button onClick={() => imageDisplayRef.current?.resetZoom()} className="w-8 h-8 text-green-400 hover:bg-green-500/20 rounded transition-colors text-sm" aria-label="Reset Zoom">1:1</button>
        </div>
      )}
